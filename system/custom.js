 ...
 */
	function showAreaP(pid){
		if(pid){
			$.post("/index.php?do=ajax&view=area",{pid: pid}, function(html){
				var str_data = html;
				if($(".pop_province").length>0){
					if (trim(str_data)==''){
						$("#city").html('<option value=""> 请选择市 </option>');
						$("#area").html('<option value=""> 请选择县或区 </option>');
					}else{
						$("#city").html(str_data);
						$("#area").html('<option value=""> 请选择县或区 </option>');
					}
	 ...
	 * @param 
	 */
	function showArea(area_pid,area_id){
		if(area_pid){
			area_id = area_id ? area_id : 0;
			$.post("/index.php?do=ajax&view=area",{area_pid: area_pid,area_id:area_id}, function(html){
				var str_data = html;
				if($(".pop_province").length>0){
					if (trim(str_data) == '') {
						$("#area").html('<option value=""> 请选择子行业 </option>');
					}
					else {
						$("#area").html(str_data);
					}
