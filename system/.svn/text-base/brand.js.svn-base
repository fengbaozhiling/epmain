 ...
 */
		...
		$('.br_info a').hover(function(){
			...
			if($('#'+sid).length==0){
				$('<div class="bl_info clearfix fl_l" id="'+sid+'">')
						...
						.load("/index.php?do=ajax&view=brand&ac=service&sid="+sid);
			}
			$('#'+sid).show();
		});
	})	
  $(function(){
	...
	$('.re_on_off').click(function(){
        ...
        ajaxTab('jcarousel2', '', "/index.php?do=ajax&view=talent&s_uid=" + $(this).attr('uid'));
...
function brand(brand){
	if(check_user_login()){
		if(shop_id>0){
			var url = '/index.php?do=ajax&view=brand&ac=apply&brand='+brand;
			showWindow('brand',url,'get','0');
			return false;
		}else{
			showDialog('您还未开通店铺,无法申请加入品牌馆,点击确认去开通','confirm','操作提示',function(){
				location.href='/index.php?do=user&&view=space';
			});
		}
	}
}